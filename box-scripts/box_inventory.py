...
def traverse_folder(folder_id, path):
    ...
    for item in items:
        if item.type == 'folder':
            ...
            file_records.append({
                "Activity Date": file.created_at,
                "Activity Type": "Created",
                "Folder Name": folder_label,
                "Folder Path": path,
                "File Name": file.name,
                "File ID": int(file.id),  # Cast ID to integer for sorting
            ...
            # Log Modified activity (only if different)
            if file.modified_at and file.modified_at != file.created_at:
                file_records.append({
                    "Activity Date": file.modified_at,
...
# Sort strictly on File ID only
df.sort_values(by='File ID', inplace=True)

today = datetime.today().strftime('%Y%m%d')
output_path = f"/Users/nic/Desktop/boxVDR_ActivityLog_{folder_code}_{today}.xlsx"

wb = Workbook()
ws = wb.active
ws.title = "Box Activity Log"
